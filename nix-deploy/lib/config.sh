...
validate_flake_ref() {
  ...
  # Split flake path and attribute path
  local flake_path="${flake_ref%%#*}"
  local attr_path="${flake_ref#*#}"

  # Expand relative paths
  if [[ "$flake_path" =~ ^\. ]]; then
    flake_path="$(pwd)${flake_path#.}"
  fi

  log_debug "Validating flake reference: $flake_ref"
  log_debug "  Flake path: $flake_path"
  log_debug "  Attribute path: $attr_path"

  # First, check if the flake is valid
  if ! nix flake show "$flake_path" >/dev/null 2>&1; then
    log_warn "Flake path invalid or not accessible: $flake_path"
    return 1
  fi

  # If there's an attribute path, validate it exists
  if [[ "$flake_ref" == *"#"* ]] && [[ -n "$attr_path" ]]; then
    # Check if the specific attribute exists
    if nix eval "${flake_path}#${attr_path}" --apply 'x: true' >/dev/null 2>&1; then
      log_info "Flake reference valid: $flake_ref"
      return 0
    else
      log_warn "Attribute path not found in flake: $attr_path"

      # Try to show available homeConfigurations for helpful error
      local available
      available=$(nix flake show "$flake_path" 2>/dev/null | grep -A 10 "homeConfigurations" | grep "├───" | awk '{print $2}' || true)

      if [[ -n "$available" ]]; then
        log_warn "Available homeConfigurations:"
    ...
    # No attribute path, just validate the flake itself
    log_info "Flake path valid: $flake_path"
    return 0
  fi
}
