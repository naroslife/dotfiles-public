...
# Get activation package path
ACTIVATION_PACKAGE=""
if [[ -f "activation-package.path" ]]; then
    ACTIVATION_PACKAGE=$(cat activation-package.path)
elif [[ -f "metadata.json" ]] && command -v jq >/dev/null 2>&1; then
    ACTIVATION_PACKAGE=$(jq -r '.store_path' metadata.json)
else
    echo_error "Cannot determine activation package path"
    echo_info "Please run import-closure.sh first"
    exit 1
fi
...
if [[ ! -e "$ACTIVATION_PACKAGE" ]]; then
    ...
    echo_info "Please ensure the closure was imported successfully"
    exit 1
fi
